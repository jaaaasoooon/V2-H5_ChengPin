...
namespace BoqiangH5
{
    ...
    public partial class UserCtrlBqBmsInfo : UserControl
    {
        ...
        public void HandleRecvMcuSNEvent(object sender, CustomRecvDataEventArgs e)
        {
            BqUpdateMcuInfo(e.RecvMsg);
        }

        public bool isReadsn = false;
        //public event EventHandler RequireReadSnEvent;
        string barcode = string.Empty;
        public void ReadSnOver(string sn)
        {
            barcode = sn;
            ReadDeviceInfo();
        }
        ...
        public void SetOffLineUIStatus()
        {
            ...
            epBarcode.Fill = new SolidColorBrush(Colors.LightGray);
        ...
        private bool IsMatchBarcodeRule(string barcode)
        {
            ...
            else if(str7 == "04" || str7 == "06" || str7 == "09" || str7 == "11")
            {
                int day;
                if (int.TryParse(str8, out day))
                {
        ...
        public void ShowDetection(bool isSoftwareOK,string softwareVersion,bool isHardwareOK,string hardwareVersion)
        {
            this.Dispatcher.BeginInvoke(new Action(() =>
            {
                ...
                if(barcode != "0")
                {
                    if (barcode.Length == 16)
                    {
                        if(IsMatchBarcodeRule(barcode))
                        {
                            epBarcode.Fill = new SolidColorBrush(Colors.Green);
                        }
                        else
                        {
                            epBarcode.Fill = new SolidColorBrush(Colors.Red);
                        }
                    }
                    else
                    {
                ...
                {
                    isWriteSnInfo = false;
                    CSVFileHelper.SaveSNInfomation(SnRecordPath, tbBarcode.Text.Trim(), softwareVersion, hardwareVersion, m_ListBmsInfo, m_ListCellVoltage);
                }
            }));
        }
        ...
        public void HandleRecvDeviceInfoDataEvent(object sender, CustomRecvDataEventArgs e)
        {
            if (readDevice)
            {
                ...
                else
                {
                    ...
                    //SnList.Add(tbBarcode.Text.Trim());
                }
                readDevice = false;
            }
        }
        public bool isWriteSnInfo = false;
